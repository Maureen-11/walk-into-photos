...
工程可以运行，当前已接入照片支持质量路线；真实室内侧视仍需视觉复核。不要把生成结束、测试通过或候选 GLB 当作真实空间质量通过。最新回归为后端 **57 passed**、前端 `pnpm run build` 通过、`pip check` 通过；10 张素材已在本地模型环境逐张完成 Moondream 分类与 MoGe 质量路线生成。I01 已完成真实 API／浏览器／file:// 离线路线验证，但视觉质量仍保守标为待复核。P14 安全切换基础层保留，旧记录继续可读。
