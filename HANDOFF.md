...
工程可以运行，当前已接入照片支持质量路线；真实室内侧视仍需视觉复核。不要把生成结束、测试通过或候选 GLB 当作真实空间质量通过。最新回归为后端 **55 passed**、前端 `pnpm run build` 通过、`pip check` 通过；10 张素材已在本地模型环境逐张完成 Moondream 分类与 MoGe 质量路线生成。I01 已完成真实 API／浏览器／file:// 离线路线验证，但视觉质量仍保守标为待复核。P14 安全切换基础层保留，旧记录继续可读。
...
本轮新增路线是“照片支持质量场景”：有效图片统一进入本地规划；质量请求默认恢复 MoGe 照片投色表面，再按类别补充有限实体结构与共享碰撞布局。模型失败时才登记 `procedural_coarse`，并保留 `fallback_reason`。通用展厅不再是默认质量路线。
...
当前新电脑事实修正：实测为 RTX 4060 Laptop、显存约 8188 MiB；MoGe 与 Moondream 已在隔离模型环境本地运行，并完成缓存后的离线读取复测。Moondream 已补充真实 API 代表图验证；MoGe 真实重建画面、浏览器性能和第二机兼容性仍未完成质量验收；本次交付采用不依赖模型的程序化粗模主路线。
