...
from app.services.pixel_scene import build_pixel_nature_v22


def main() -> int:
    parser = argparse.ArgumentParser(description="Build the isolated Q05 N01 fence-composition candidate.")
    parser.add_argument("--input", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--scene-id", default="pixel-q05-r22-n01")
    args = parser.parse_args()
    if not args.input.is_file():
        parser.error(f"input image does not exist: {args.input}")
    payload = build_pixel_nature_v22(args.input, args.output, args.scene_id)
    ...
    print(json.dumps({
        ...
        "fence_eye_line_correction": True,
