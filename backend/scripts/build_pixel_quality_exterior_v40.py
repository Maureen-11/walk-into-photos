...
from app.services.pixel_scene import build_pixel_building_v42, build_pixel_nature_v40, build_pixel_street_v41


def main() -> int:
    parser = argparse.ArgumentParser(description="Build isolated Q03 V40 exterior fine-pixel material candidates.")
    parser.add_argument("--input", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--profile", choices=("n01", "s01", "b01"), required=True)
    parser.add_argument("--scene-id")
    args = parser.parse_args()
    if not args.input.is_file():
        parser.error(f"input image does not exist: {args.input}")
    defaults = {
        "n01": (build_pixel_nature_v40, "pixel-q05-r40-n01"),
        "s01": (build_pixel_street_v41, "pixel-q05-r41-s01"),
        "b01": (build_pixel_building_v42, "pixel-q05-r42-b01"),
    }
    ...
    print(json.dumps({
        ...
        "geometry_inherited_from": "pixel-v33" if args.profile == "n01" else "pixel-v35",
    }, ensure_ascii=False))
    return 0
