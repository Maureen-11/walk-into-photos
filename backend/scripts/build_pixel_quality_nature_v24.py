...
from app.services.pixel_scene import build_pixel_nature_v24


def main() -> int:
    parser = argparse.ArgumentParser(description="Build the isolated Q05 N01 lighting-contrast candidate.")
    parser.add_argument("--input", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--scene-id", default="pixel-q05-r24-n01")
    args = parser.parse_args()
    if not args.input.is_file():
        parser.error(f"input image does not exist: {args.input}")
    payload = build_pixel_nature_v24(args.input, args.output, args.scene_id)
    ...
    print(json.dumps({
        ...
        "geometry_inherited_from": "pixel-v23",
    }, ensure_ascii=False))
    return 0
