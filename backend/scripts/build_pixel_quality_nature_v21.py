...
from app.services.pixel_scene import build_pixel_nature_v21


def main() -> int:
    parser = argparse.ArgumentParser(description="Build the isolated Q05 N01 collision-safety candidate.")
    parser.add_argument("--input", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--scene-id", default="pixel-q05-r21-n01")
    args = parser.parse_args()
    if not args.input.is_file():
        parser.error(f"input image does not exist: {args.input}")
    payload = build_pixel_nature_v21(args.input, args.output, args.scene_id)
