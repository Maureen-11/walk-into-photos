...
from app.services.pixel_scene import build_pixel_living_v29


def main() -> int:
    parser = argparse.ArgumentParser(description="Build the isolated Q05 I02 visible-surface candidate.")
    parser.add_argument("--input", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--scene-id", default="pixel-q05-r29-i02")
    args = parser.parse_args()
    if not args.input.is_file():
        parser.error(f"input image does not exist: {args.input}")
    payload = build_pixel_living_v29(args.input, args.output, args.scene_id)
    ...
    print(json.dumps({
        ...
        "geometry_inherited_from": "pixel-v28",
    }, ensure_ascii=False))
    return 0
