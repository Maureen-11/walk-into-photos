...
from app.services.pixel_scene import build_pixel_living_v15


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the isolated Q03 I02 camera-facing surface readability candidate."
    )
    parser.add_argument("--input", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--scene-id", default="pixel-q03-r11-i02")
    args = parser.parse_args()
    if not args.input.is_file():
        parser.error(f"input image does not exist: {args.input}")
    payload = build_pixel_living_v15(args.input, args.output, args.scene_id)
